...
bool Log::init(const char* file_name, int close_log, int log_buf_size,
    ...
    time_t t = time(NULL);
    struct tm* sys_tm = localtime(&t);  // 将日历时间转换成本地时间
    struct tm my_tm = *sys_tm;

    const char* p = strrchr(file_name, '/');    // 找出字符串中最后一次出现的字符c的地址，然后将该地址返回
...
void Log::writeLog(int level, const char* format, ...) {
    ...
    va_list valst;              // 指向参数指针
    va_start(valst, format);    // ap指向参数v之后的那个参数的地址，即ap指向第一个可变参数在堆栈的地址
    ...
    int n = snprintf(m_buf, 48, "%d-%02d-%02d %02d:%02d:%02d.%06ld %s ",
                     my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_mday,
                     my_tm.tm_hour, my_tm.tm_min, now.tv_sec, now.tv_usec, s);
    // 写入内容
